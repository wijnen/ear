...
class Markers(Gtk.TreeView):
	def __init__(self, gui):
		...
		self.target = None
		self.endtarget = None
		self.store = Gtk.TreeStore(object, str, str, str, GdkPixbuf.Pixbuf, str, GdkPixbuf.Pixbuf)
		Gtk.TreeView.__init__(self, self.store)
		self.markers = [self.render_icon(Gtk.STOCK_MEDIA_FORWARD, Gtk.IconSize.MENU, None), self.render_icon(Gtk.STOCK_MEDIA_PLAY, Gtk.IconSize.MENU, None)]
		self.stoppers = [self.render_icon(Gtk.STOCK_GOTO_TOP, Gtk.IconSize.MENU, None), self.render_icon(Gtk.STOCK_GO_DOWN, Gtk.IconSize.MENU, None), self.render_icon(Gtk.STOCK_GOTO_BOTTOM, Gtk.IconSize.MENU, None)]
	...
	def mkcol(self, title, col, icons = False):
		...
		if icons:
			...
			tvcol.pack_start(renderer[0], True)
			tvcol.pack_start(renderer[1], True)
	...
	def recompute_target_impl(self, node):
		...
		if not self.row_expanded(self.store.get_path(node)):
			...
			return
		c = self.store.iter_children(node)
		if c is None or self.target is None or self.store.get_value(c, 0).start > self.target:
			if self.store.get_value(node, 4) != self.markers[1]:
				self.store.set_value(node, 4, self.markers[1])
			while c:
				self.clear_targets(c)
	...
	def get_current(self):
		...
		return None
	def show_marker(self, markeredit):
