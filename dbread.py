...
def read(use_cache = True, unique_names = False):
	...
	# Add all other files.
	for dirname in basedirs:
		for orig_root, dirs, files in os.walk(dirname, followlinks=True):
			root = os.path.abspath(orig_root)
			assert orig_root.startswith(dirname)
			subdir = orig_root[len(dirname):] + os.sep
			if subdir.startswith(os.sep):
				subdir = subdir[len(os.sep):]
			for filename in files:
				if makepath(root, filename) in used:
					continue
				if os.path.splitext(filename)[1] not in exts:
					continue
				tracks.append(add_unfragmented_file(subdir + filename, dirname, usednames))
				used.add(makepath(subdir + filename, dirname))
